...
def push_dummy_gh_branch(repo, branch, keyfile):
    """
    Makes a dummy commit on a given GitHub repo and a given branch

    Requires that the branch does not exist. Keyfile should be an absolute path.

    Should be used as a contextmanager, it will delete the branch and the clone
    directory when done.
    """
    ...
    with tempfile.TemporaryDirectory() as gitdir:
        ...
        try:
            ...
            subprocess.check_call(
                ["git", "push", "--delete", "origin", f":{branch}"],
                env=git_env,
                cwd=gitdir,
            )
...
def test_build_binder(binder_url):
    ...
    with push_dummy_gh_branch(
        ...
        branch,
        os.path.abspath(".secret/hub23-ci-key"),
    ):
